...
resource "local_file" "vscode_sftp" {
  ...
  content = replace(
    templatefile(
      ...
      {
        agent = fileexists("/Users/max.schaefer/Library/Group Containers/2BUA8C4S2C.com.1password/t/agent.sock") ? "/Users/max.schaefer/Library/Group Containers/2BUA8C4S2C.com.1password/t/agent.sock" : "/run/user/1000/ssh-agent.socket"
        devices = local.devices
        servers = local.servers_merged
      }
    ),
